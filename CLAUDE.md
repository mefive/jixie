...
- Factor、Strategy、Research、Market、Signals 五个核心业务模块按业务能力分目录；根级保留 `README.md`、`schema.ts` 和按需存在的共用业务 `errors.ts`。本业务 Agent 启动与上下文放在 `agent/`，Job 定义与提交、查询放在所属业务目录，测试跟随实现。纯来源类型、快照与哈希不依赖任务执行模块；Factor 分别归 `sources/snapshot.ts` 和 `sources/fingerprint.ts`。不要求五个模块具备相同目录，不将此布局套用到 Agent、Engine、Auth、Sharing 或 Application Maintenance。
