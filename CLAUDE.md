...
- Factor、Strategy、Research、Market、Signals 五个核心业务模块按业务能力分目录；根级保留 `README.md`、`schema.ts` 和统一的业务 `errors.ts`。本业务 Agent 启动与上下文放在 `agent/`，Job 定义与提交、查询放在所属业务目录，测试跟随实现。纯来源类型、快照与哈希不依赖任务执行模块；Factor 分别归 `sources/snapshot.ts` 和 `sources/fingerprint.ts`。不要求五个模块具备相同目录，不将此布局套用到 Agent、Engine、Auth、Sharing 或 Application Maintenance。
...
- **业务入口使用具名输入类型**：已有业务 schema 对应的对象参数，在同模块 `schema.ts` 中紧邻 schema 导出派生类型，业务文件使用 `import type` 引用，不在导出的业务函数签名中内联 `z.input`、`z.output` 或 `z.infer` 推导表达式。HTTP、Agent 工具、SDK 适配等入口解析外部参数，业务操作接收 `z.output`，保留权限、状态和业务规则检查；持久化 JSON、Job/Worker 消息、LLM 输出仍须在各自读取边界校验。确有原始输入消费者时使用 `z.input`，未知数据解码器可接收 `unknown`；类型名的 `Input` 表示业务入参，不决定选哪一种。保持现有共享契约与独立业务类型；局部推导、协议/Job/工具类型按实际归属保留，不为没有类型消费者的 schema 批量添加别名，不新增集中式 `types.ts`。详见 `docs/design/api-input-types.md`。
