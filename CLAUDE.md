...
- **基本面研究职责**：基本面研究平台负责来源、字段口径、历史可得时间、缺失提示、公式实现和结果可复现；用户选择分析方法、预测假设和投资结论。估值模板与市场验证是可选研究工具，不以策略盈利或价格兑现作为平台能力交付条件。 这不降低正式 Factor/Strategy 的既有准入标准。
...
- Strategy TS 公开签名、文档和 Agent 参考来源为 `packages/shared/src/sdk/strategy/reference.ts`。
...
## Factor SDK Contract 工作流

- Factor TS 的公开签名与双语说明归 `packages/shared/src/sdk/factor/reference.ts`；Monaco 使用其声明渲染器，
  同目录 `contract.ts` 由 `setup:sandbox` 生成，SDK 实现与 runtime 元数据直接消费该类型，禁止另写一份前端签名。
- Factor Python 的公开类型和字段以 `packages/shared/src/sdk/factor/python.ts` 为唯一真相源；Prisma 的 Factor
...
- Factor、Strategy、Research、Market、Signals 五个核心业务模块按业务能力分目录；根级保留 `README.md` 和统一的业务 `errors.ts`；仅有后端专属校验组合或内部入参时保留 `schema.ts`。本业务 Agent 启动与上下文放在 `agent/`，Job 定义与提交、查询放在所属业务目录，测试跟随实现。纯来源类型、快照与哈希不依赖任务执行模块；Factor 分别归 `sources/snapshot.ts` 和 `sources/fingerprint.ts`。不要求五个模块具备相同目录，不将此布局套用到 Agent、Engine、Auth、Sharing 或 Maintenance。

- **执行职责按业务组织**：无状态业务编排优先使用具名函数，不为单一 `run` 方法创建 Runner 类，不强制各模块采用同一目录或签名。Worker 负责输入/消息/进程收尾，语言 runtime 负责代码加载与执行，不规划报告、扫描或文档运行。类用于确实持有资源或可变状态的 runtime、会话池和进程执行器；共享模拟/计算放在明确的业务能力目录，避免混入语言 runtime。具体调整见 `docs/design/execution-naming-and-ownership.md`。
...
- 业务模块的多组 HTTP 路由统一放在 `routes/`，由 `routes/index.ts` 组合并具名导出，外部调用方显式导入该入口（如 `#strategy/routes/index.js`），不保留根级转发文件。实现按职责命名为 `definition.ts`、`agent.ts`、`backtest.ts` 等，不重复 `-routes` 后缀。只有一组路由的模块可以直接在根级 `routes.ts` 实现。五个核心业务模块的整体接口测试放在 `routes/index.integration.test.ts`（Market 使用 `index.test.ts`），专属路由测试与实现同放 `routes/`；其他模块沿用其现有测试布局。子路由直接导入子实现，避免反向导入 `index.ts` 形成循环。模块路由出口允许显式具名 re-export，不建立应用级总 barrel。业务错误统一定义在模块根级 `errors.ts`，由 `infra/http/errors.ts` 在 `server.ts` 的 `onError` 统一映射，子路由不另设错误映射；公共 HTTP 请求 schema 与类型位于 `packages/shared/src/api/<业务>.ts`，路由直接导入；后端专属校验组合留在模块根级 `schema.ts`。鉴权、维护门禁等中间件从模块 `middleware.ts` 单独具名导出，不经路由入口转导出；`infra/http` 保留跨业务 HTTP 辅助。路由对象统一使用「业务/职责名 + `Route`」具名导出（如 `authRoute`、`strategyBacktestRoute`），调用方直接使用同名导入，不导出通用 `routes` 再用 `as` 命名。路由只负责请求/响应，资源归属、状态与事务由业务入口负责。
...
- `apps/api/src/jobs` — 通用任务存储、日志、事件调度和 Worker 回收；register.ts 显式注册七类 JobLifecycle 对象；JobService 统一存储、执行、终态事务与恢复。onExecute 在事务外计算，onSuccess/onFailure/onInterrupted 在终态事务中更新业务，onCommitted 在提交后执行。bootstrap 等待恢复后启动单消费调度器；buildApp 只构建 HTTP。报告运行状态来自 Job，无 Job 历史终态回读 legacy 字段；ResearchExecution 保留领域结果状态。
...
- `apps/api/src/engine` — simulation 为交易循环与账户，data 为必填 DataPort/EngineData，factors 为引擎内因子求值；adapters 为宿主 Prisma 与 TS/Python FactorHost，testing 为 fixture。因子源码只在各自沙箱执行，Engine 经独立 FactorExecutionPort 使用计算结果。模拟核心不导入宿主适配器；TS/Python 策略经共享 bridge 使用宿主 Engine，TS bundle 只包含 SDK 与沙箱适配。见 `src/engine/README.md`。
...
- `apps/api/src/maintenance` — 整轮更新顺序、在途任务等待、发布水位、运行恢复与应用可用性协调；定时触发和锁由 systemd/Shell 提供。财报子进程、ETF 补齐及数据质量规则归 Market 所属业务；维护通过回调保存 checkpoint，并组合发布检查和整体审计；`routes.ts` 实现并导出状态路由，`middleware.ts` 单独导出维护门禁；publication/risk-audit.ts 组合市场数据和策略模型要求，publication/audit.ts 汇总审计。公共锁与在途任务等待在 runs/coordination.ts，不从 daily.ts 导入；财报 Worker 逐项完成须等父进程持久化确认后才能继续。调用 Market 同步入口，流程见 `src/maintenance/README.md`。
...
- `packages/shared` — 共享类型、公开 SDK Contract 和 HTTP 请求契约；依赖方向 `apps/* → packages/*`，反向禁止。HTTP schema 通过 `@jixie/shared/api/<业务>` 子路径导入，不进入根级运行时 barrel；前端只使用 `import type` 引用请求类型。
- `packages/shared` 编译到 `dist`，实际运行／构建前需 `pnpm --filter @jixie/shared build`（install 时 `prepare` 也会自动构建）。根级 `pnpm typecheck` 在内存中生成最新 shared 声明再检查各应用，不运行构建、不写文件、不依赖旧 dist；各应用保持自己的 TypeScript 配置。

应用 CLI 放在所属模块的 `cli/`，负责参数、配置、调用、输出、退出与进程资源收尾，允许简单同步组合和只为输出服务的查询；可复用业务不导入 CLI，不为每个命令新增转发 service。Market / Maintenance / Signals / Auth 的入口统一登记在 `apps/api/scripts/README.md`。独立备份工具 `apps/api/scripts/backup-db.mjs` 保留 Node 内置模块 + sqlite3 CLI 的零 npm 依赖契约，不要求应用构建。
...
`pnpm check:backend-boundaries` 先运行检查器自测，通过后扫描项目，放在人工 review 通过的行为验证阶段执行。根级 build/typecheck 直接调用底层静态扫描，不运行自测。静态扫描不执行应用模块；Worker、Python、Prisma 和 Pyright 资源路径须按 `docs/backend-runtime-entries.md` 做实际验证。

## 代码约定

- **ESM 相对导入必须带 `.js` 后缀**(即使源是 `.ts`)
- **API 跨顶层模块导入使用原生包内别名**：例如 `#jobs/service.js`、`#market/instruments/stock-identity.js`，根级日期辅助用 `#date`；模块内部保留相对路径，跨 workspace 继续使用包名。映射唯一来源为 `apps/api/package.json#imports`，新增可跨模块引用的顶层目录时同步补充。Worker/资源的 `new URL(..., import.meta.url)` 仍使用相对路径，不当作模块导入替换。
...
- **业务入口使用具名输入类型**：已有业务 schema 对应的对象参数，在 schema 所属文件中导出派生类型（公共 HTTP 契约在 shared/api，后端专属组合在业务 `schema.ts`），业务文件使用 `import type` 引用，不在导出的业务函数签名中内联 `z.input`、`z.output` 或 `z.infer` 推导表达式。HTTP、Agent 工具、SDK 适配等入口解析外部参数，业务操作接收 `z.output`，保留权限、状态和业务规则检查；持久化 JSON、Job/Worker 消息、LLM 输出仍须在各自读取边界校验。前端请求 `*Request` / `*RequestQuery` / `*RequestParams` 使用 `z.input`，与后端业务 `z.output` 区分；查询字符串在序列化处接受共享类型检查，coerce 必须显式声明字符串输入类型。未知数据解码器可接收 `unknown`；类型名的 `Input` 表示业务入参，不决定选哪一种。保持现有共享契约与独立业务类型；局部推导、协议/Job/工具类型按实际归属保留，不为没有类型消费者的 schema 批量添加别名，不新增集中式 `types.ts`。详见 `docs/design/api-input-types.md` 与 `docs/design/api-request-contracts.md`。
- **业务错误统一约定**：Strategy、Factor、Research、Signals、Market、Agent、Auth、Maintenance 的业务错误在各自根级 `errors.ts` 定义，继承 `infra/errors.ts` 的 `BusinessError`；调用点直接 `throw new XxxError(reason, options)`，不增加只负责抛出的 `failXxx` 函数。成功返回数据，预期拒绝抛异常；合法的空查询和运行状态仍是数据。错误定义只含稳定 reason、分类、i18n key/参数、可公开 details 与内部 cause，不依赖 HTTP、数据库或业务执行。自定义技术异常也放所属模块根级 `errors.ts`，保留取消、重试和诊断语义，不冒充业务拒绝。HTTP 统一映射：输入 400、缺失 404、状态冲突 409、认证 401、权限 403、暂不可用/维护 503；未知异常记录原始错误并返回安全双语 500。不得按文案识别错误或 catch 所有异常后转 400。非 HTTP 输出在消费边界用 `errorMessage(error, locale)` 翻译。详见 `docs/design/api-errors.md`。
- **代码注释一律用英文**(inline `//`、块注释、JSDoc、Prisma `///`、CSS `/* */`)——维护者可能不识中文,注释不留中文括注,用标准英文财经术语。例外(仍/可中文):**i18n 资源里的 zh 值**、**CLAUDE.md / README 文档**。**LLM prompt / 工具 description / few-shot 也一律英文**(见下「多语言」条)。**面向用户的 UI/报错文案走 i18n**(英文 key,zh+en 值),不再硬编码——详见 `docs/design/i18n.md` 与下「多语言」条
...
回测已由 `strategy/backtests/strategy-backtest-lifecycle.ts` 派发到 `strategy/backtests/worker`，参数扫描位于 `strategy/scans`，Factor 与 Signals 也有各自的线程/子进程入口。HTTP 主线程负责提交与结果事务，Engine 负责模拟；长任务沿用「同步写库 → 返 jobId → 后台计算 → 查询/订阅进度」。具体入口和资源收尾见 `docs/backend-runtime-entries.md`。

## Commit message policy

- All commit subjects and bodies must be written in English using `type(scope): description`.
