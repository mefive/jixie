...
> **替代状态（2026-09-14，删除收尾已通过审查和验证）**：由[嵌入式 Python 分析](embedded-python-analysis.md)接替新对话计算。
> 本轮代码已删除 `analyzeDataTool`、`gen-stats-doc.ts` 及专用生成链，保留正式业务使用的统计库和历史图表需要的沙箱。
> 本文保留原始设计记录；沙箱、路由和预算等细节不代表当前实现，当前事实与删除进度见扩展规划及代码。
