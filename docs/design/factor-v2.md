...
6. **不新增债券或商品专用回测引擎。**统一策略 Lab 继续负责目标权重、`period` 调仓、成本、成交约束
   和每日交易信号。
7. **预测因子与风险因子明确分开。**Phase 0–4 已完成收益预测因子闭环，Phase 5 已把久期、信用、
   通胀等风险暴露和归因接入 Strategy Lab；风险分析仍不与 Alpha 检验混在同一报告里。
...
| Risk Factor 风险因子 | 用于解释收益、风险和相关性的系统驱动，如久期、信用、通胀 | 组合风险模型维护 |
...
**2026-08-11 Phase 5 Strategy Lab 风险闭环：**多资产回测完成交易核算后，由 API 主进程加载严格
availability-aligned 的市场风险驱动和冻结 Factor 报告，再把可用证据写入
`AllocationAnalysis.risk`。交易引擎与策略代码不读取这些结果；风险研究失败只记录系统日志，不能抹掉
已经完成的回测。日频组合收益用于 252 日市场暴露，月度组合收益用于五轴宏观敏感度；横截面/Panel
Factor 复用批准报告的费后多空逐期收益，单一时间序列 Factor 则明确使用实际策略月度归因流，不伪造
报告级多空收益。压力情景只在严格 PIT 的 Market Risk 段存在时生成。

Lab 的“多资产配置归因”新增统一“风险研究”页签，分别展示市场暴露与方差贡献、宏观敏感度、
Alpha/Risk 重合和确定性/历史压力情景，不增设债券或商品专用回测页面。宏观历史最新值报告保留为
...
完成边界已经闭合；后续真实 vintage 积累和新增专业数据属于持续演进，不反向把最终值报告改称 PIT。
...
伪装为实时 vintage。该切片当时完成的是数据契约、同步、bootstrap 和质量审计，尚未把最终修订历史值
接入宏观状态 evaluator；随后以明确 revision policy 的 as-of loader 和未来函数测试通过了进入
Macro Regime 报告的硬门槛。
...
历史在本地首次捕获前返回空，2026-08-09 当日状态点则不含未来 vintage。随后把状态历史与目标 ETF 的
未来收益、风险、持续时间和切换频率组合成 Macro Regime Report，而没有直接接入策略权重。
...
Phase 5 随后单独完成风险因子与组合归因：

- 久期、曲线、信用、权益 beta、通胀、美元和商品风险暴露；
- 因子收益归因、风险贡献和情景冲击；
- Alpha Factor 与 Risk Factor 的重合检查。

**验收结果：**商品连续收益能够解释换月规则；宏观报告冻结 revision policy，并将最终值历史明确限制为
不可发布的探索证据；风险归因没有被包装成收益预测结论。完整真实浏览器证据见 Phase 5 收口验收。
...
| D：专业固收商品与宏观 | Phase 5 | Carry、库存、宏观状态、风险暴露和归因 | 完成 | 曲线、实际合约、仓单、PIT 宏观 |

原计划以里程碑 B 作为最小可独立交付边界；实际实施继续完成了 C 和 D。用户现在既可走通“发现时间序列
因子 → 样本外验证 → 发布 → ETF 策略 → 每日交易信号”，也可完成跨资产 Panel、宏观状态研究和策略
组合风险解释。后续增加数据与特征仍遵循需求拉动，不再作为 Factor V2 工程阶段延长计划。
...
Phase 5 不曾作为 MVP 或 Multi-Asset 的阻塞条件，随后已按第 8 节的独立完成边界收口。
研究到交易的闭环始终优先于继续扩充因子数量、增加复杂优化器或建设单独的债券/商品研究页面。

### 14.3 Phase 5 专业研究完成定义

Phase 5 在以下边界内完成：

1. 商品 Carry、仓单和会员持仓等专业数据具有可审计来源、单位、可得日与换月规则；
2. 增长、通胀、流动性、信用和外部压力进入统一宏观状态/风险协议，并披露 revision policy；
3. Strategy Lab 能解释市场暴露、风险贡献、宏观敏感度、Alpha/Risk 重合和预设压力情景；
4. 缺少严格历史 vintage 的宏观结果只能作为探索证据，不能发布或冒充样本外结论；
5. 风险研究不读取或改变交易决策，也不新增债券/商品专用回测页和期货交易范围。

### 14.4 2026-08-11 最终收口状态

Factor V2 Phase 0–5 已按上述完成定义全部冻结为完成。最终回归同时覆盖：

- 商品 Carry 时间序列研究；
- 国债曲线 Factor 到策略回测、部署和每日信号；
- 跨资产 Panel 合成、策略引用、成交及收益/风险归因；
- 仓单压力研究的资产适用范围、共同数据截止、报告与研究/交易隔离。
- 增长—通胀宏观状态研究及 latest-vintage 发布门禁；
- 九项市场风险暴露、五轴宏观敏感度、Alpha/Risk 重合与线性压力情景。

Factor V2 至此整体完成。5/60 日仓单变体、仓单 Panel、校准预测值等是尚未登记研究假设的可选增强，
真实宏观 vintage 需要随时间自然积累；它们不构成未完成阶段。商品/国债期货交易和机构级风险模型是
独立产品方向，不属于 Factor V2 范围。
