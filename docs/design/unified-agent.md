...
> 2026-09-10 更新：本文保留历史设计与完成记录。策略 Agent 快速回测已退役，当前职责是代码生成、只读查询和必要校验，回测由用户在工作台显式发起；Research 通过封存研究生成 Strategy 草稿。Factor 探索分析保留，见 [当前产品决策](agent-research-loop.md#当前决策策略回测回到工作台2026-09-10)。

> 2026-07-06 设计,对应原 `ROADMAP.md` 主线七。**2026-07-06 当日四阶段实施完成**，当前归入已完成基础 C；本文保留为设计依据。方向:lab / factor / screen 三处共用**一个 agent 核心**,按场景注入 profile(prompt / 产物校验器 / 工具集);agent 获得**只读数据工具**(从「盲写」到「先查再写」),并能回复**查询卡片**;screen 页从「NL 输入框 + 结果表」降级改造为「卡片墙」。
