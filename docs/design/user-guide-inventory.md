...
基线 `aa838133`。中英文各 100 篇，覆盖 11 个目录组；100 个 slug 均已登记。正文已通过人工 review、静态检查、Docs 构建与现有帮助站 E2E。下表主要记录源码和内容核对，不表示各项业务计算已实跑；线上部署未核对。第 2 项已完成本地隔离环境制图与图片分类，人工 review、Docs 构建与现有帮助站 E2E 已通过。
...
### 第 2 项截图交付（已通过 review 与验证）

以 `337eea15` 为正文基线，逐张检查原有 152 张引用图片。本轮用真实页面制作 28 个场景的中英双语图，共 56 张：覆盖策略输入／草稿参数／报告选择与复核、Research 文档／搜索／归档／四类数据目录／运行控制／对话引用、Factor 报告问答／发布入口／用于策略预填、新建菜单、多部署／暂停、市场、估值、对象详情、因子气象与公共库入口。

逐图处置和引用文章见 [user-guide-images.json](user-guide-images.json)：

| 处置 | 数量 | 边界 |
| --- | --- | --- |
| 当前界面重拍 | 56 张（28 对） | 其中 11 张覆盖旧中文路径；英文正文引用真实英文 UI 图。模板、名称、代码等用户内容可能仍为中文，图注说明 |
| 历史结果／状态示例 | 105 张 | 原数值不变，两种语言均标明历史中文界面；84 张裁掉顶部 40px 的旧导航文字，21 张保留局部图或完整边界以免裁掉标注／抽屉标题 |
| 学习路径原始证据 | 29 张 | 像素和 SHA-256 均与基线一致，只补历史图注与当前操作链接 |
| 登录界面保留 | 3 张 | 当前邮箱／邀请码／验证码控件仍一致；英文图注明示中文 UI |
| 退出引用并移除 | 4 张 | 旧首次参数图、发布／归档混淆图、单部署空状态图、旧钉住窗口，分别由当前图替代 |

制图使用开发库的一次性本地副本与当前应用，原库未修改；只通过正常接口建立未运行模板、归档示例和部署／暂停状态，不调用模型、不生成信号、不重算收益。回测与 Factor 的结果来自已有真实报告；公共库、数据集没有记录时保留真实空态，并在图注中说明。

素材限制：一份旧 Research 文档因历史 JSON 字段无法由当前 Prisma 读取；未更改其源码、输出或业务实现以绕过错误，改用正常接口创建的当前模板展示编辑与保存。嵌入式分析本轮拍摄当前引用入口，不宣称拍摄了成功计算、输入留存或版本历史；相关行为仍以已有文字与专项功能验收为准。因子气象历史卡片、完整研究运行和成交／信号历史保留原始示例。

复拍入口：`pnpm docs:images refresh`，前提与配置见 [help-content-refresh.md](../../apps/web/e2e/help-content-refresh.md)。脚本只访问显式指定的本地一次性环境；账号信息与记录 ID 在库外配置。原始脱敏图位于 `apps/web/acceptance/help-refresh/{zh,en}/`，编号图进入 `apps/docs/public/images/help/{zh,en}/`。制图完成不等于业务计算通过回归测试；人工 review 后已完成 Docs 构建与现有帮助站 E2E。

静态结果：100 对文章图片顺序一致，865 处内部链接及 193 张引用图片均可静态解析；清单引用／校验值和 29 张学习原图校验通过。Docs lint／typecheck、制图脚本 lint／语法、格式及差异检查通过。截图专用服务和数据库副本已清理。共享包与 Docs 构建、现有帮助站 E2E 均通过。E2E 使用独立空库与真实服务，覆盖 100 篇中文文章、关键页面语言切换、图片预览、公式/代码和 390px 窄屏；桌面及英文窄屏截图已目检。测试中将报告历史页旧图片数量断言更新为三张具体图片路径，完整重跑通过。验收临时服务、数据库及连接均已清理；第 3 项已完成完整双语遍历和只读图片清单自动校验，见下文。

### 第 3 项验收与维护入口

三提交范围已完成。`pnpm --filter docs check:help` 只读核对注册表、中英文正文、865 处内部链接、193 张引用图片、图片处置清单和 29 张学习原图校验值；`pnpm --filter docs test:help-content` 的 10 项有效／失效样例全部通过。后续更新文档或复拍图片时必须同步相应清单，校验器不会自动接受新的校验值。

`pnpm e2e docs-help` 通过真实浏览器逐篇检查中英文各 100 篇的标题、图片路径／图注／加载状态和内部文章链接，实际点击六组重点工作流链接，并检查移动端文章菜单与 390px 布局。Docs lint、typecheck、格式／差异检查、shared 和 Docs 构建通过；双语桌面与英文窄屏截图已目检。临时空库环境不调用模型、不重算金融结果，已关闭服务并删除临时库。具体命令与边界见 [E2E 说明](../../apps/docs/e2e/README.md)。本地完成不表示已 push 或线上部署。

### 全部文章登记

下表状态表示本提交的正文处理，不表示每条业务流程已实跑。中文和英文按同一 slug 成对核对；图片处理与双语文档运行验收已由后两项完成。
...
| 项目 | 内容 |
| --- | --- |
| 页面入口 | 市场、因子气象、策略工作台、选股看图、因子研究、估值、今日信号 |
...
| 项目 | 内容 |
| --- | --- |
| 路由 | `/strategy`、`/strategy?id=<策略编号>`、`/strategy?new=1` |
| 页面操作 | 新建策略；输入一句话描述；使用示例；直接写代码；打开历史策略；修改策略；处理未运行修改提示；让策略 Agent 快捷试算当前代码 |
| 使用手册文章 | 认识策略工作台；新建策略；一句话生成策略；Agent 快捷回测；使用示例；直接写代码；历史策略；修改后重新运行；未运行修改提示 |
...
| 当前截图 | `workspace-01.png`、`strategy-description-01.png`、`strategy-generated-01.png`、`strategy-revised-01.png`、`strategy-revised-result-01.png`、`edit-rerun-01.png`；历史验收目录中的 `4-strategy-hero.png`、`4b-strategy-cards.png`、`4c-code-editor.png`、`4c1-factor-hover.png`、`4d-new-dirty-confirm.png`、`learn-6-strategy-hero.png`、`7r-sdk-hover.png` |
...
| 项目 | 内容 |
| --- | --- |
| 页面操作 | 从策略工作台使用 ETF 示例；在选股看图中直接查询 ETF；运行 ETF 策略；查看 ETF 交易明细 |
| 使用手册文章 | 查询 ETF；运行 ETF 策略；ETF 与股票策略的差异 |
| 当前 E2E | `etf-trading.mjs`、`help-content-etf-futures.mjs` |
| 当前截图 | `etf-entry-01.png`、`etf-trades-01.png`；验收目录中的 `etf-1-strategy-entry.png`、`etf-2-research-entry.png`、`etf-3-trade-detail.png` |
...
| 当前 E2E | `learn.mjs` |
| 当前截图 | `learn-1-top.png` 至 `learn-6-strategy-hero.png` |
...
| 下一步 | 替换 | 使用帮助首页 | 改为产品完整阅读路径，不只链接 SDK 和策略工作台 |
...
阶段 L 以 `d64224e` 为基线完成后续用户功能补录：新增研究文档与 Cell、数据目录、研究输出、运行控制、
Research Agent 协作、研究交接、研究整理、Python Factor、稳健截面推断、公共资产库和技术指标 11 篇
中英文正文，帮助中心扩展到 88 篇；新增 17 张真实功能标注图，并重跑研究工作台、Python Factor、稳健
推断、技术指标和公共资产库专项 E2E。文档站全量 E2E 逐篇核对正文、图片、站内链接、公式、代码页签和
窄屏布局；浏览器目检发现并关闭图片 figure 嵌套段落导致的 hydration 警告。数据库迁移、供应商同步、
任务队列、沙箱实现和内部治理协议不进入用户操作正文。阶段 H 继续暂缓。

阶段 M 以 `a129ffe` 为基线完成研究入口、口径确认、美国国债收益率曲线和固定 Python 统计环境补录：
新增研究口径确认、收益率曲线和 Python 研究运行环境 3 篇中英文正文，帮助中心扩展到 91 篇；同步更新
第一次研究、研究文档、Research Agent 协作、数据目录和研究输出等既有文章。新增 6 张真实功能标注图，
并重跑口径确认专项 E2E、新增收益率数据与 SciPy／statsmodels／原生图表专项 E2E。文档站全量 E2E、
build、lint、Research SDK／runtime contract 检查和浏览器目检通过。数据库迁移、AST 校验、Contract
生成、运行环境安装和启动脚本不进入用户操作正文。阶段 H 继续暂缓。
