...
把「个人无法发明因子」翻成「一张可逐个本地检验的菜单」。每个预置因子带:定义、预期方向、一句出处、A 股注意事项。候选清单(按数据可得性排):

| 因子 | 定义 | 预期方向 | 数据 | 备注 |
|---|---|---|---|---|
| 12-1 动量 | 过去12月剔近1月收益 | 学术正,A股弱/失效 | 已有 | 与现 mom 参数变体 |
| 低波动 | 近N月收益波动率 | 低波占优 | 已有(vol 反向) | low-vol anomaly |
| Amihud 非流动性 | \|日收益\|/成交额均值 | 高非流动溢价 | 已有 | 与 size 高相关,必配中性化 |
| 换手率(拥挤) | 近N日换手均值 | 高换手负(彩票偏好) | 已有 | A 股散户特色,预期显著 |
| ROE 质量 | 最新年报 ROE / 5年稳定性 | 正 | fina_indicator 已同步 | FactorBar 需补 roe(已在 backlog) |
| 毛利率 | 毛利/营收 | 正(质量) | 需 income 表 | 数据依赖,后做 |
| 应计 accruals | (净利−经营现金流)/资产 | 高应计负 | 需资产负债+现金流表 | 数据依赖,后做 |
| 北向持股变化 | 陆股通持股比例变动 | 正(聪明钱) | 需 hk_hold 同步 | 数据依赖,后做 |
