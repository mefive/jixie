...
> 2026-07-06 设计,对应原 `ROADMAP.md` 主线三，当前归入已完成基础 A / B。目标:打通「因子页验证 edge → 策略里直接使用」的闭环,并把个人研究者的方法论纪律固化进工具。
...
> 零 schema 迁移)。实况与验收见原 `ROADMAP.md` 3.1/3.1b;本文保留为设计依据。
...
- 性能预期与预置价格因子同级(月度全区间 ~2 分钟级,已持久化报告秒开)——已有流式日志/worker/Job 管道,无新工程。
...
1. **key / 报告稳定性**:预置现在的 key 是 `ep`/`mom`(URL `?factor=`、`FactorReport.factor` 都用它)。迁成 `Factor` 行后 id **必须仍是这些稳定 slug**,否则老报告全 orphan、分享链接全断。→ 预置用**固定 slug**(`ep`/`mom`…),不用 ULID;自定义继续用 ULID。
...
基本面走硬编码原生函数(`fundFn(r)`)比走编译沙箱 `compute()` 略快:ep 全市场月度 ~5000 股 ×~130 期 ≈ 65 万次 sandbox 调用。自定义分支已这么跑、实测可接受,但迁完 ep 会比现在慢几秒(持久化报告秒开,可忽略)。

### 验收

- 9 个预置全部从 `Factor` 表(builtin 行)读代码运行,`factors.ts` 的两个注册表 + 3 条硬编码分支删除,`computeFactorSeries` 只剩一条 compile+compute 路径。
- 迁移前后同因子同参数 IC/分层逐位一致;老 `FactorReport`(按 `ep`/`mom` 记录 factor)不 orphan。
...
- 报告 spec 扩展:组合定义(因子列表+权重+标准化方式)进入版本化 `AnalysisSpec` 和 `variantKey`。
...
- **行业分类:申万一级(SW2021,31 个),以此为准**。前置需一次数据同步(2026-07-08 拍定):
  ...
  - 同步脚本 `src/market/cli/sync-sw-industry.ts`:`index_member_all` 按 31 个 `l1_code` 逐个拉全成分(不加 `is_new` 过滤,要**全历史**含 `out_date`),幂等 `deleteMany`+`createMany`。数据量 ~1 万行,一次拉完。
...
- **接口/报告**:参数 `neutral: 'none' | 'size' | 'size_industry'`(默认 none),进入版本化 spec、
  `variantKey` 和 `FactorReport` payload;UI 分析参数条第三个选择器。旧报告回填时缺省为 none。
...
## 3.5 预置因子库扩充(经典因子菜单，2026-08-12 已以 20 个收口)

把「个人无法发明因子」翻成「一张可逐个本地检验的菜单」。每个预置因子带:定义、预期方向、一句出处、A 股注意事项。候选清单(按数据可得性排;**状态列 = 台账**,验证数字取各候选冻结的探索/holdout 区间,IC/ICIR(年化)/费后年化取「市值+行业中性化」口径除非注明):
...
| 毛利率稳定性(504日) | PIT 毛利率阶梯序列总体标准差，至少 4 段报告 | 负(越稳定越好) | fina_indicator ✓ | **已否决(2026-08-12)** | 探索中性化 IC −0.0213 通过，但一次性 holdout IC −0.00940 未达冻结门槛 `< -0.01`；IC>0 52.94%、费后高减低 +4.30% 亦不支持负向，不改线、不准入 |
...
| 市场残差波动率(20日) | 个股日收益对中证全指日收益带截距回归的残差总体标准差 | 高残差波动负 | `ctx.history('marketClose')` ✓ | **已收录(2026-08-12)** | 探索 IC −0.08379 / ICIR −2.81，holdout IC −0.09844 / ICIR −2.68，均通过 `< -0.01`；与 `vol120` 相关 0.6353 < 0.90 冗余线；单指数近似，不称完整 FF3 IVOL |
| 月内最大单日收益(21日) | 最近 21 个日收益的最大值 | 高 MAX 负(彩票偏好) | 已有价格历史 ✓ | **已收录(2026-08-12)** | 探索 IC −0.06875 / ICIR −2.80，holdout IC −0.08712 / ICIR −2.69；与 vol/vol120/resid_vol20 相关 0.8613/0.5425/0.8251，均过 `<0.90` 冗余门；换手 73%～77%，强预测但高成本 |
| 销售收益率(1/PS_TTM) | `PS_TTM > 0` 时取倒数 | 正(价值) | daily_basic ✓ | **已收录(2026-08-12)** | 探索 IC +0.03775 / ICIR +1.65，holdout IC +0.03469 / ICIR +1.54；与 ep/bp 相关 0.4695/0.5919；正式段费后 +0.66%，低换手温和增量 |
| ROA 盈利能力 | 最新已披露报告 ROA(annDate PIT) | 正 | fina_indicator ✓ | **已否决(2026-08-12)** | 探索 IC +0.01496 过主门，但与 ROE 相关 0.94984 ≥ 0.90，判定换皮；未消耗 holdout |
| 资产负债率 | 最新已披露报告 debt_to_assets(annDate PIT) | 事前负 | fina_indicator ✓ | **已否决(2026-08-12)** | 探索 IC +0.00575，未过 `< −0.01` 且符号相反；不翻方向，未做相关性或 holdout |
| Amihud 非流动性 | \|日收益\|/成交额均值 | 高非流动溢价 | 需 ctx.history 扩 'amount' | 待 SDK | 与 size 高相关,必配中性化 |
| 异常换手率 | 21日自由流通换手均值 / 252日均值 | 高异常换手负(彩票偏好) | daily_basic.turnover_rate_f ✓ | **已收录(2026-07-13)** | 复刻书中/Liu et al.(2019) PMO 排序变量;新增 PIT 日频字段与 `ctx.history(n, 'turnoverRateF')`,待全量回填后验证 |
...
维护形态:候选清单直接列在本文档(上表),带状态列(待数据/待验证/已收录/已否决),执行模型每验证一个更新一行——文档是正式台账上线前的临时索引;正式台账见 3.6 改造后的不可变 `FactorReport`。

## 3.6 研究纪律:不可变 FactorReport + 保留期 + 多重检验

> 2026-07-14 历史化基础已按 `docs/design/factor-report-history.md` 实施。该文档是本阶段的准确契约;
> 研究卡、holdout 与多重检验的下一阶段详设见 `docs/design/factor-research-discipline.md`。

### 数据模型:报告就是实验,Job 就是 attempt

已落地的数据模型:

- `id`:ULID,不再用复合缓存键当主键;
- `variantKey`:对 `{ spec, factorCodeHash, dataRevision }` canonical JSON 后 sha256;
- `factor/factorCodeSnapshot/factorCodeHash`:保存当时正文与 hash,Factor 后续编辑不影响旧报告;
- `specJson`:版本化 `AnalysisSpec` 快照;
- `status`:`running | done | error | stale`,`phase` 当前为 `legacy | explore`;
- `parentReportId` 已记录普通重跑血缘,后续通过 `phase=holdout` 区分正式保留段关系;
- 既有 `factor/freq/start/end/neutral/payload/computedAt` 保留用于查询 / 展示,`payload` 是当次完整报告。

新报告的输入和代码快照不可修改。旧报告保留原复合主键并回填 `phase=legacy`、V1 spec 和可取得的当前
代码快照;新 URL 使用 `?factor=<id>&report=<reportId>`。

`Job.factorReportId` 与报告一一对应。相同变体仅在 running 时返回现有 report/job;终态后重跑会新建二者。
worker 只计算冻结输入,父进程按 reportId 将 payload 与 report/job 终态放在同一事务中持久化。服务启动时
遗留 running job 及其报告一起标为 stale。

`dataRevision` 不能只等于最大交易日:同一截止日的数据可能回填 / 修正。实现前在 4.7 数据审计 / 同步
流程中确定一个单调市场数据 revision(或等价快照指纹),并放入 variantKey 输入。当前字段预留为空。

### 研究卡、Holdout 与多重检验

- **不新增 `FactorExperiment`**:`FactorReport` 就是一次 experiment;只有未来出现命名研究、成组变体、
  独立结论 / 归档 / 分享时才提炼上层对象。
- 第一次探索运行前冻结轻量研究卡。允许纯探索,但运行后不能改写快照把故事补成先验。
- 默认探索由服务器按市场最新交易日留出最近 18 个月;正式 holdout 使用父报告冻结代码和非日期参数,
  新 report 以 `phase=holdout,parentReportId=<探索报告>` 关联。结果先封存,显式揭示后写不可逆 `revealedAt`。
- 多重检验新增 `testKey = hash(spec + factorCodeHash + 结构化预设判据)`,与包含 data revision 的精确执行
  `variantKey` 分开。N 按完成 explore 的不同 testKey 计数;同输入同判据 retry、基础设施失败和单纯
  数据修复不重复计。
...
- `universe`:上市年限、流动性剔除、历史 ST / 风险警示、待退市、负净资产、长期停牌;每级输出样本数。
  历史状态不可得时显示 `unavailable`,不以当前状态污染历史。
- `missing`:窗口因子的 `minCoverage`(默认 2/3)和日期 / 交易观测语义;动量与波动率不得共用一种填充法。
- `outlier`:factor exposure 与 forward return 两份独立配置。暴露支持 none / winsor / MAD,在中性化 OLS
