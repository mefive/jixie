...
## 嵌入式 Python 分析（2026-09-14，开发验收完成）

Factor / Strategy 保留对话内的即时数据验证，计算与图表统一接入 Research Python。首次成功固定版本，
每次尝试留存，旧解释引用确定运行，用户可继续到 Research。预设及只读因子问答补齐会话与报告上下文。
新路径已接入；删除收尾已移除 `analyzeDataTool`、统计说明生成脚本及专用生成链，保留历史图表所需兼容设施。
删除收尾已通过代码审查、统计/兼容回归、构建及中英用户流程，实际覆盖与限制见下方扩展文档。

该扩展不恢复快速回测，不替代 Factor 已有受约束探索，也不自动揭示 Holdout、发布或执行账户操作。
范围及提交顺序见[嵌入式 Python 分析](embedded-python-analysis.md)。以下 2026-09-10 决策与验证记录仍描述已完成工作。
