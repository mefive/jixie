...
## 2026-09-18：统一策略与因子执行边界（四个提交均验证通过）

本轮按 review-gated-development 执行原定三个提交及一个性能补充提交；每个提交先确认范围，实现后只做静态检查，
人工 review 通过再跑行为验证，全部通过后提交。此节取代下文历史的“TS 因子跟随 Engine 执行”决定；
原定三个提交均已完成实现与验证；扩展压力测试发现重复 ensureBars 的历史传输回退，第四提交已完成增量传输修复与验证。
...
| 3 | `refactor(strategy): run both language runtimes through the host engine` | 已提交 `04f62a16`；人工复审、静态检查、回归、构建、源码/编译 Worker、基准及五项 E2E 通过 |
| 4 | `perf(strategy): avoid resending cached history on ensureBars` | 人工 review、静态检查、回归、构建、源码/编译 Worker 及两种场景基准通过 |
...
### 第四提交：ensureBars 增量传输（2026-09-20，验证通过）

背景：第三提交后增加合成压力测试，每组一次冷启动、两次预热、五次正式采样；48 次净值/成交哈希
全部一致。1000 标的 × 252 日期的正式总耗时中位数旧/新为 1888.95 / 2359.59 ms；100 标的 ×
1260 日期为 1014.20 / 1349.95 ms；300 标的池、每日轮换 50 标的并 ensureBars 为 529.16 /
1832.53 ms（3.46 倍）。动态组新版累计传输 275,507,448 字节。不是完整旧版本快照或真实数据库
负载，时间不作通用性能保证；上述是第四提交修复前的数据。

本提交：

- `bridge.ts` 的 ensure_bars 分派复用每个 code 的已传输日期；首次请求仍传完整可见历史，重复
  请求只补尚未同步的日线，当日已同步则空增量。每日快照继续更新已登记标的。
- 每次请求仍调用 Engine 的 ensureBars，因此数据加载、因子准备和错误响应继续执行。加载失败
  不登记为已同步；空历史首次初始化，后续正常更新。不修改 Python 历史协议、公开 SDK 或数据范围。
- 新增协议帧测试覆盖同日重复、新旧标的混合、日期间隔/停牌、加载失败重试、空历史及 Engine
  调用次数；isolate 用例检查重复 ensureBars 后窗口不重复、返回副本不可污染内部历史。
- 基准加入动态场景，分别运行固定 `f276bfbd` 旧墙内实现、固定 `04f62a16` 修复前 runtime/bridge、
  当前实现。动态场景 300 标的池、252 日期，每日窗口轮换 20 个标的、请求 50 个、SMA20 排序持有
  前 10 个，每组一次冷启动、两次预热、五次采样；小 watch 基准保留原采样方式。验证净值/成交哈希
  和传输字节下降，耗时作为诊断记录，不设机器相关的耗时门槛。
- 本次是一个补充 commit，信息固定为 `perf(strategy): avoid resending cached history on ensureBars`。
  没有数据库迁移、HTTP、公开 SDK、workspace 或跨包构建依赖变更；大股票池/长周期的其他开销不在本提交优化范围。

人工 review 通过后的验证结果：

- Strategy runtime、Engine 因子语义及源码 Worker 共 114 项通过；两项显式开关基准另行运行并通过。
  编译 Worker 8 项、边界检查器自测 28 项通过；Shared/API 构建通过。
- 回归初次失败源于测试夹具直接发送含重复代码的协议帧，违反既有 schema；仅修正夹具，保留真实
  isolate 中 SDK 重复代码输入的覆盖。固定旧版基准模块的路径提取改用 path.relative，避免尾部斜线
  导致截掉首字符。两处均为测试代码修正；人工 review 后未修改产品代码。
- 两种基准独立于其他重负载检查执行，共 50 次运行，各场景内所有版本净值/成交哈希一致。
  小 watch 正式总耗时中位数旧墙内实现 103.22 ms、当前实现 102.71 ms，未见明显回退。

动态场景结果（每组五次正式采样，总耗时包含初始化、执行和释放）：

| 实现 | 总耗时中位数 | 正式采样范围 | 每轮累计传输量 |
| --- | --- | --- | --- |
| 旧墙内 Engine（f276bfbd 入口） | 522.59 ms | 487.50–551.63 ms | 未按同一口径统计 |
| 修复前宿主 Engine（04f62a16 runtime/bridge） | 2315.93 ms | 2063.75–2446.60 ms | 275,507,446 字节 |
| 当前增量传输 | 491.57 ms | 488.74–532.91 ms | 50,267,488 字节 |

相对修复前，总耗时中位数下降 78.8%，累计传输量下降 81.8%。当前与旧墙内实现的采样区间重叠，
不能据此宣称所有策略都更快。此基准沿用第三提交的受控架构对照：固定旧入口及修复前 runtime/bridge，
其他依赖使用当前实现，不是完整旧版本 checkout；合成内存行情不包含真实数据库 I/O。进程启动和
夹具创建不计时，各版本保留进程内 bundle 缓存，最大 RSS 为整进程峰值。大股票池、长周期的其他
开销未在本提交优化或重新测量。

源码/编译 Worker 测试均等待退出并清理独立数据库；基准临时模块目录已清理。本提交未启动测试服务，
未重跑浏览器 E2E；第三提交的五项 E2E 结果保留在上节。

静态检查通过：pnpm typecheck 的五个 workspace 全部通过，生成物一致性通过；后端 780 个文件、
2919 条运行时边、690 条类型边，0 违规。11 处非字面量导入已登记（本次增加固定旧 runtime 的
测试导入）。变更 TS/MJS 的 ESLint、Prettier 与 git diff --check 通过。
