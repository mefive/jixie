 ...
 * hand-written; contract.ts is generated from the same signatures and consumed by the implementation.
 * Static adapter checks and regression tests guard the Engine and language boundaries.
 */
...
export const SDK_ENTRIES = [
  ...
  },
  {
    ...
    zh: '决策日有效的申万一级行业标签，如银行／食品饮料；无覆盖区间时为 null。用于无行业分类前视的行业中性、轮动或行业限定。',
    en: 'Point-in-time SW level-1 industry label effective on the decision date; null without a covering spell. For sector-neutral or rotation logic without classification lookahead.',
  },
  {
    iface: 'StrategyCtx',
    name: 'lhbNet',
  ...
  },
  {
    ...
    name: 'name',
    group: '业务类型 BarRow(整行字段)',
    sig: 'name: string | null',
    zh: '该交易日生效的证券名称;历史名称缺失时为 null。',
    en: 'Point-in-time security name effective on this trading day; null when unavailable.',
  },
  {
    iface: 'BarRow',
    name: 'riskWarning',
    group: '业务类型 BarRow(整行字段)',
    sig: 'riskWarning: boolean',
    zh: '该交易日是否处于 ST、*ST 等风险警示状态。',
    en: 'Whether the stock was under an ST, *ST, or equivalent risk warning on this day.',
  },
  {
    iface: 'BarRow',
    name: 'pendingDelisting',
    group: '业务类型 BarRow(整行字段)',
    sig: 'pendingDelisting: boolean',
    zh: '该交易日是否处于退市整理命名状态。',
    en: 'Whether the stock was in a delisting-period naming state on this day.',
  },
  {
    iface: 'BarRow',
    name: 'open',
...
export function buildSdkDts(
  ...
  return buildDeclarations(locale, buildFactorKeyType(locale, factorOptions), docLink);
}

/** Generate implementation types from the same signatures as editor declarations.
 * Published factor keys are resolved per user at runtime, so this package cannot enumerate them. */
export function renderStrategySdkContract(): string {
  const declarations = buildDeclarations('en', 'type FactorKey = string;').replace(
    /declare function defineStrategy([\s\S]*?)\): void;/,
    'type DefineStrategy = $1) => void;',
  );
  return (
    '// Generated by scripts/sandbox/setup.ts from reference.ts. Do not edit by hand.\n' +
    declarations.replace(/^(interface|type) /gm, 'export $1 ')
  );
}

function buildDeclarations(
  locale: Locale,
  factorKeyType: string,
  docLink?: (name: string) => string,
): string {
