...
本页不是只给代码。2026-08-25，我们在一份全新的 Research 文档中完整运行了下文的固定案例，并用专用浏览器
...
| 关系 | 全样本相关 | 区块 bootstrap 95% 区间 | 36 个月滚动范围 |
| --- | ---: | ---: | ---: |
| 沪深 300／恒生 | 0.6865 | [0.5957, 0.7838] | [0.5955, 0.8500] |
...
这次固定运行的证据，不承诺数据修订或样本变化后数字永远相同；专用验收会继续检查样本、恒等式、三组区间和图表
是否仍能真实生成。
