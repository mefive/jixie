...
两组回测和完整风险归因，并生成本页截图。验收结束后，策略已删除，临时发布的 Factor 已归档。
...
轮动把最大回撤从 -40.72% 降到 -13.27%，但累计、年化和 Sharpe 都低于基线。滑点损耗约 7.66 万元，也远高于
基线。固定裁决是：**有历史防守效果，但没有证明收益或风险调整后收益胜出，不应作为生产候选。**
...
3. 用同日期、资金和成本建立可比较的买入持有基线；
4. 从资产和资产类别两层对账收益、费用与风险贡献；
5. 正确解读滚动相关、利率环境、市场／宏观风险和压力情景；
6. 在回撤改善但收益、Sharpe 和成本不合格时给出否定结论。
...
基线与轮动必须使用相同的 2018-01-01 至 2026-07-30、100 万元和成本参数。否则回撤与收益不可直接比较。
...
证据：Panel Rank IC 为 0.0181，但成本后多空年化 -8.25%。轮动最大回撤从基线
-40.72% 降到 -13.27%，但年化只有 0.37%、Sharpe 0.088，均低于基线；
