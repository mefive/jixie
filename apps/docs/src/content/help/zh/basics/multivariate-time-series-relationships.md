...
在[研究工作台](/research)中写清问题与方法，用[数据目录](/docs/help/research/data-catalog)取数，在 Python Cell 中显式完成对齐、统计和图表；也可在 Factor／Strategy 对话中使用[嵌入式分析](/docs/help/research/embedded-analysis)。下文解释可选择的方法，不表示平台会自动执行所有检验或生成固定报告页签。以本次实际运行的代码、输入和输出为准。

## 为什么建议预先指定一个核心变量

模型可以估计所有变量的系数。为减少事后挑选，建议主要结论回答一个预先指定的问题，例如：

> 控制 CPI 后，实际利率变化是否仍与黄金月收益负相关？

实际利率是核心变量，CPI 是控制变量。若要正式回答“控制实际利率后 CPI 是否与黄金相关”，应在查看结果前把 CPI 改为核心变量并作为另一份研究运行。这是研究纪律，需要在文档中显式记录；自由 Python 计算不会自动强制这一约束。
...
偏 R² 应报告数值及其经济含义。如果需要“较小”或“较大”等标签，应在计算前说明阈值；当前自由研究不会自动套用统一等级。

## “控制其他变量后的关系”图是什么

可以在 Python 中分别从结果变量和核心变量中剔除控制变量可以线性解释的部分，再用散点图比较两组残差。它依据 Frisch–Waugh–Lovell 定理，与完整回归中的核心变量系数相同。
...
其中 `R_j²` 来自“用其余解释变量解释第 j 个变量”的回归。VIF 接近 1 表示共线性弱；若选择 5 作为诊断阈值，应在代码中显式计算并显示提示。高 VIF 不意味着必须机械删除变量，而是说明系数和符号可能对样本区间敏感，应回到研究逻辑判断变量是否重复表达同一概念。

相关矩阵展示解释变量之间的两两 Pearson 相关。它能指出明显重复维度，但不能替代 VIF，因为多个变量的组合也可能产生共线性。

## HAC 区间、残差和滚动系数

可以在 Python 中对全部系数使用 Newey–West/HAC 稳健标准误，并记录滞后阶数。它调整异方差和有限阶序列相关造成的区间偏差，但不改变 OLS 系数，也不能修复错误的变量、共同趋势或结构突变。

残差一阶自相关较高时，说明模型可能遗漏动态结构。滚动核心系数用固定窗口重复估计核心变量的偏回归系数，用于检查方向是否只在某段时期成立。窗口过短会导致多变量估计噪声很大。

## Python 教学示意

下面代码假定已经通过 SDK 准备同频、同口径的 `sample` 表，并设置 `hac_lag`；样本构造、滞后与缺失处理需预先说明。这里的 `sample` 是数据表，避免覆盖平台的 `data` 取数对象：
