...
样本很大时，很小的差异也可能有很小的 p 值。因此不能只根据“显著/不显著”下结论，还应在代码中计算并同时检查效应量和方向。

## 极端值敏感性

估值比率等指标常有长尾。可在代码中把两组各自上下尾的极端值压到预先指定的分位数，然后重新计算缩尾均值差。如果原始均值差和缩尾均值差变号，结论对极端值敏感，不应被强解读。

## Python 教学复现

示例假定 `group_a`、`group_b` 已来自同一时点、相同口径的数据，且已处理缺失与非有限值。取数和样本构造需在前面的 Cell 中完成；下面不是可直接独立运行的完整文档。
