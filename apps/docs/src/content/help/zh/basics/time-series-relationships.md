...
3. **不完整期间**：研究完整月收益时应显式排除未结束的当前月，并核对 SDK 参数；只有明确研究“月初至今”时才应包含。
4. **滞后**：不平移是同期关系；预测研究需在代码中把解释变量移到结果变量之前，并检查实际日期。
...
金融序列常有异方差和时间相关。可以在 Python 中使用 Newey–West/HAC 标准误调整斜率的不确定性，并显式设置滞后阶数；这会改变标准误、
...
下面代码假定已经准备好名为 `predictor`、`outcome` 的两个序列，并预先设置 `hac_lag`。取数、频率与滞后处理需在上游 Cell 中完成；结果以本次实际代码、输入和参数为准：
