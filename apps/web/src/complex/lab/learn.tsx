      ...
      <div className="jx-learn-layout">
        ...
        <main className="jx-learn-main">
          ...
          <p className="jx-learn-abstract">
            ...
            <code>bar</code>/<code>bars</code> 和下单的 <code>order</code>/<code>target</code>
          ...
          <Section id="quickstart" num="01" title="五分钟:你的第一个策略">
            ...
            <p className="jx-learn-p">
              ...
              —— 买入按 100
              股整手、当日买次日才能卖(T+1)、开在涨停板买不进、佣金印花税都算了。这些你一行没写,是引擎替你强制的。
            </p>
            <Why>
              不用写任何 <code>import</code>。<code>defineStrategy</code> 和 <code>ctx</code>{' '}
          ...
          <Section id="onbar" num="02" title="onBar:策略的心跳">
            <p className="jx-learn-p">
              引擎按交易日一天天往前走,
              <b>
                每个交易日调用一次你的 <code>onBar(ctx)</code>
              </b>
              。 关键是那个 <code>ctx</code>:它<b>永远绑定「今天」</b>,没有 date
              参数。所以一天里发生三件事:
            </p>
            <ol className="jx-learn-steps">
              ...
              <li>
                <b>调 onBar,你做决定</b> —— 你能读到的最新数据截止今天;下的单进队列,
                <b>留给明天开盘</b>成交。
              </li>
            </ol>
            <Why>
              为什么 <code>ctx</code> 没有 date 参数、决策和成交要隔一天?因为这从
              <b>物理上杜绝了未来函数</b>:
          ...
          <Section id="data" num="03" title="读数据:bar 一横排,bars 一纵列">
            <p className="jx-learn-p">
              名字只差一个 <code>s</code>,却是两个世界。<code>bar</code> 是
              <b>今天全市场里某只票的一整行</b>(横切),
            ...
            <p className="jx-learn-p">
              技术指标 <code>ctx.sma / ema / atr / highest</code> 都是在 <code>bars</code>{' '}
              那一列历史上算出来的,所以它们和 <code>bars</code> 一样,需要该票的 K 线
              <b>先加载进来</b>。加载有两条路:
            </p>
            <pre className="jx-learn-code">{CODE_LOAD}</pre>
            <Why>
              最常见的新手坑:<code>await ctx.universe()</code> 拿到一批 code 后直接{' '}
              <code>ctx.sma(code, 20)</code>, 结果全是 <code>null</code>。因为{' '}
              <code>universe()</code> 只加载了「今天这一横排」,没加载每只票的纵向历史。
          ...
          <Section id="orders" num="04" title="下单:order 是动作,target 是目标">
            <p className="jx-learn-p">引擎给两种下单范式,一根 bar 里用哪套由你定:</p>
            <table className="jx-learn-table">
              <thead>
                <tr>
                  <th></th>
            ...
            <p className="jx-learn-p">
              为什么选股要用 <code>target</code> 而不是自己 <code>order</code>
              ?因为月度调仓时新旧持仓有重叠,用 order 你得自己算
              「哪只清、哪只减、哪只加、各多少股」;而 <code>target</code> 你只描述<b>想要的终局</b>
              ,差额(先卖非目标、再买)引擎替你做。
            </p>
            <Why>
              <code>target</code> 是<b>全量快照,不是增量改单</b>:你交出的这份清单<b>就是</b>
              完整目标, 任何没列进去的持仓一律按目标 0 处理——<b>会被卖光</b>。所以别只写想动的那只(
              <code>setHoldings(&#123;A:0.5&#125;)</code>
              会清掉 B、C);想清空整个组合,传空 <code>setHoldings(&#123;&#125;)</code>{' '}
              即可。而只清一只、别的不动,用命令式的 <code>ctx.exit('A')</code>。
            </Why>
          </Section>
          ...
          <Section id="walkthrough" num="05" title="实战:一步步搭一个截面选股">
            <p className="jx-learn-p">
              目标:<b>每月</b>在沪深 300 里挑 <b>ROE&gt;15 且最便宜(EP 最高)的 30 只,等权持有</b>
              。一步步来。
            </p>
            <p className="jx-learn-p">
              <b>第一步 · 每月只跑一次。</b>引擎每天都调 onBar,但调仓是月度的,用{' '}
              <code>ctx.period</code> 配一个 <code>let last</code> 守卫,月份没变就直接返回:
            </p>
            <pre className="jx-learn-code">{CODE_STEP1}</pre>
            <p className="jx-learn-p">
              <b>第二步 · 选票。</b>拿今天的沪深 300 成分作候选池,链式地过滤 → 排序 →
              取头部,再等权下单。 注意用了 <code>universe</code> 的 onBar 必须是 <code>async</code>:
            </p>
            <pre className="jx-learn-code">{CODE_STEP2}</pre>
            <p className="jx-learn-p">
              就这样。<code>where</code> 过滤、<code>rankBy</code> 按 EP(1/PE)从高到低、
              <code>top(30)</code> 取前 30、
              <code>equalWeight</code> 等权建仓——剩下的调仓、成交、成本全归引擎。这个例子只用了{' '}
              <code>bar</code> 的截面字段; 如果你的信号要均线,记得在 <code>top</code> 之后{' '}
          ...
          <Section id="next" num="07" title="下一步">
            <p className="jx-learn-p">你已经会写两类策略了。接下来:</p>
            <div className="jx-learn-cards">
              <a className="jx-learn-card" href="/docs">
                ...
                <div className="jx-learn-cardDesc">
                  按符号查 API:<code>ctx</code> 的每个方法、<code>Universe</code> 链式操作、
                  <code>BarRow</code> 全字段。
                </div>
              </a>
