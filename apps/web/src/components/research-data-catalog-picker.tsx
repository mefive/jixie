...
import {
  researchBacktestReportSelection,
  researchDatasetSelection,
  researchFactorReportSelection,
  researchFactorWeatherSelection,
  researchSeriesSelection,
  researchStrategyScanReportSelection,
  type ResearchCatalogSelection,
} from '@src/complex/research/research-data-catalog';
import './research-data-catalog-picker.css';

interface ResearchDataCatalogPickerProps {
  model: {
    dataCatalogLoader: LoaderModel<ResearchDataCatalogResultV1>;
    searchDataCatalog(
      query: string,
  ...
  onSelect(selection: ResearchCatalogSelection): void;
  actionLabel?: string;
  intro?: string;
  open: boolean;
  onClose: () => void;
...
export const ResearchDataCatalogPicker = reactUtils.observer(
  ({
    open,
    onClose,
    model: store,
    ...
    actionLabel,
    intro,
  }: ResearchDataCatalogPickerProps) => {
    ...
    const selection =
      view === 'backtest_reports'
        ? selectedStrategyScan
          ? researchStrategyScanReportSelection(selectedStrategyScan)
          : selectedBacktestReport
            ? researchBacktestReportSelection(selectedBacktestReport)
            : null
        : view === 'factor_reports'
          ? selectedFactorWeather
            ? researchFactorWeatherSelection(selectedFactorWeather)
            : selectedReport && !selectedReport.sealed
              ? researchFactorReportSelection(selectedReport)
              : null
          : view === 'datasets'
            ? selectedDataset &&
    ...
    const insert = () => {
      ...
      const label =
        view === 'instruments' && selected
          ? i18n.language.startsWith('en')
            ? selected.nameEn || selected.nameZh
            : selected.nameZh
          : view === 'datasets' && selectedDataset
            ? localizedDatasetName(selectedDataset, i18n.language)
            : view === 'factor_reports'
              ? (selectedReport?.factorName ?? selectedFactorWeather?.factorName)
              : view === 'backtest_reports'
                ? (selectedStrategyScan?.strategyName ?? selectedBacktestReport?.strategyName)
                : undefined;
      onSelect({
        ...selection,
        reference: {
          ...selection.reference,
          label: (label ?? selection.reference.label).slice(0, 160),
        },
      });
      onClose();
    };
    ...
    return (
      <Drawer
        ...
        extra={
          <Tooltip title={actionLabel ?? t('dataCatalog.insert')}>
            <Button
              type="primary"
              shape="circle"
              disabled={!snippet}
              icon={<FontAwesomeIcon icon={faArrowRightToBracket} />}
              aria-label={actionLabel ?? t('dataCatalog.insert')}
      ...
      >
        <p className="jx-researchDataCatalog-intro">{intro ?? t('dataCatalog.intro')}</p>
