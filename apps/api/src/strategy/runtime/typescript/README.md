...
宿主先把用户源码转 CommonJS，再创建 isolate，挂载取数／因子执行／日志桥并加载 bundle。运行 finally 关闭 FactorHost、释放 isolate；TS/Python 因子均通过宿主桥在各自独立沙箱执行，但不能把用户 TS 移到宿主直接执行。因子权限由 [factor-inputs](../../factor-inputs/README.md) 在此前检查。
