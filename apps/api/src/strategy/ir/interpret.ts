...
/** Compile any strategy IR into an engine Strategy. Pipelines carry `stages`; the legacy archetype
 * carries `type`. */
export function interpretStrategy(ir: StrategyIR): Strategy {
  if ('stages' in ir) return interpretPipeline(ir);
  if (ir.type === 'cross_section') return interpretCrossSection(ir);
  throw new Error('unsupported strategy IR');
}
