...
TS/Python 均由 FactorRuntime.start 选择实现，从 metadata 读取声明，临时实例在 finally close；返回的是可传输模块和 FactorDependency，不把活动会话交给扫描 cell。Panel 单因子保留批准报告资产范围，组合解码并校验批准快照、哈希和研究范围，不能改用当前可编辑组合。模块顺序与引用键顺序对应。
