...
路由解析 body 和路径 ID，补齐 `dataReferences` 后传入 `StrategyAgentInput`。业务入口检查本人策略和运行中 turn；随后并行读取 [context.ts](context.ts) 的 `syncedIndexContext`、`publishedFactorContext`，捕获服务端嵌入上下文，构造 Strategy profile 并交给通用 Agent。上下文读取会查询数据；整个准备和入队没有包成一个总事务。
