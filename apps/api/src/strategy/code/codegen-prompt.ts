...
- **Published research factors** are referenced by immutable ID as \`release:<ULID>\`; use the same value in both the top-level \`factors\` array and \`ctx.factor\`. Never emit a new \`custom:<key>\` dependency; that legacy syntax follows mutable factor rows. **This user's active releases**: ${referencableFactors}. Experimental/validated releases are research/backtest-only; daily deployment requires active production maturity. A factor that declares a window needs the stock's K-line loaded first (ensureBars), like the built-in indicators.
