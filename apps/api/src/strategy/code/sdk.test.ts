...
import { Selection, enrich, periodKey } from './sdk.js';
...
describe('Selection', () => {
  ...
  it('where + rankBy(desc) + top(fraction) picks the cheapest decile', () => {
    ...
    const picks = new Selection(ctx, Object.keys(rows))
  ...
  it('rankBy drops codes scoring null/non-finite', () => {
    const { ctx } = ctxOf(rows);
    const ranked = new Selection(ctx, Object.keys(rows)).rankBy((b) =>
  ...
  it('dropBottom removes the least-liquid fraction', () => {
    ...
    const kept = new Selection(ctx, Object.keys(rows)).dropBottom(0.25, (b) => b.turnoverRate ?? 0);
  ...
  it('minListDays keeps unknown-age and old-enough names, drops the too-new', () => {
    ...
    const kept = new Selection(ctx, Object.keys(rows)).minListDays(365);
    expect(kept.codes().sort()).toEqual(['A', 'C', 'D']); // B (100d) dropped
  });

  it('top(N>=1) takes an explicit count', () => {
    const { ctx } = ctxOf(rows);
    expect(new Selection(ctx, ['A', 'B', 'C']).top(2)).toEqual(['A', 'B']);
  });
});
...
describe('enrich', () => {
  ...
  it('select() loads the universe into a Selection', async () => {
