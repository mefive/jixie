 ...
 */
export async function compileFactor(source: string): Promise<CustomFactor> {
  let js: string;
  try {
  ...
  try {
    // Free identifiers resolve to these params: the CJS env + the injected defineFactor; require throws.
    const run = new Function('module', 'exports', 'defineFactor', 'require', js);
    run(mod, mod.exports, defineFactor, blockedRequire);
  } catch (e) {
