...
| [etf-trend-observations.ts](etf-trend-observations.ts) `loadEtfTimeSeriesObservations` / `buildEtfTimeSeriesObservations` | 时间序列输入；loader 获取行情及利率等数据，builder 使用编译因子形成观察 |
...
loader 与 build 的区别不能简单等同于异步／纯函数：资产 builder 还会调用传入的编译因子。截止日、availableDate、预热缺值、跨资产日历及后续收益对齐必须一起核对，不能用当前最新数据填补历史缺口。因子编译对象的创建与最终 dispose 由上层执行编排负责。
