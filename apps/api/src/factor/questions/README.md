...
[conversations.ts](conversations.ts) 的 `startFactorQuestion` 由 Factor agent 路由传入 userId、factorKey、message、可选 reportId 与 locale。在事务内校验来源／报告，创建或复用会话，保存用户消息、AgentTurn 和 contextSnapshot，再启动通用 runner；首个响应已有已提交消息和 turnId。客户端不提供可信 history，旧 factorName/history 请求需刷新。
