...
[job.ts](job.ts) 的 `factorCorrelationJob` 注册为 factor-correlation，校验 payload 用户且不得关联 FactorReport。[worker.ts](worker.ts) 调用 [compute.ts](compute.ts) 的 `computeFactorCorrelation`，只复用 [横截面数据与序列](../execution/cross-sectional/README.md)，最后断开 Prisma。主线程 complete 在执行器事务中 upsert 缓存；失败／恢复保留上次成功缓存，由通用执行器更新 Job。
