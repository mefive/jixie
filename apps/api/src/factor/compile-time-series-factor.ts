...
const TIME_SERIES_FACTOR_SETUP = `
{
  ...
  if (factor.version !== 2 || factor.analysisKind !== 'time_series' || factor.outputScope !== 'asset') {
    throw new Error('Factor V2 time-series definitions require version=2, analysisKind=time_series, outputScope=asset');
  }
...
export async function compileTimeSeriesFactor(
  ...
  const module: IsolatedModule = await loadIsolatedModule({
    userJs,
    noun: 'Factor V2 code',
    injectGlobals: 'globalThis.defineFactorV2 = (factor) => factor;',
    setup: TIME_SERIES_FACTOR_SETUP,
  });

  let meta: TimeSeriesFactorDefinitionMeta;
  try {
    meta = JSON.parse(await module.callJson('meta', [])) as TimeSeriesFactorDefinitionMeta;
    validateMeta(meta);
  } catch (error) {
    module.dispose();
    throw error;
...
function validateMeta(meta: TimeSeriesFactorDefinitionMeta): void {
  if (!meta.name?.trim()) {
