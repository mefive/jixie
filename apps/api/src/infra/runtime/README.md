...
- `protocol.ts`：语言无关的通用日志/错误帧及共用字段校验，供 Strategy、Factor 和 Research 协议使用。
- `python/protocol.ts`：Python 帧包络与大小限制。`PythonFrame` 是传输包络，不代表通过了业务协议校验。
...
业务握手及交互协议分别位于 `strategy/runtime/protocol.ts`、`factor/runtime/python/protocol.ts`、`research/runtime/host/protocol.ts`；各业务 runtime 通过这些 schema 调用公共 session。公共设施不导入业务模块，也不拥有业务执行状态。
