...
业务握手及交互协议分别位于 `strategy/runtime/protocol.ts`、`factor/runtime/python/protocol.ts`、`research/sdk/protocol.ts`；各业务 runtime 通过这些 schema 调用公共 session。公共设施不导入业务模块，也不拥有业务执行状态。
