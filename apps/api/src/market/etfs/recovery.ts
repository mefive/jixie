...
import { fillEtfHistoryGap, syncEtfMarketDate } from './sync.js';
export interface EtfRecoveryOptions {
  loadCompleted?: (scope: string) => Promise<Set<string>>;
  onItemComplete?: (scope: string, item: string) => Promise<void>;
...
export async function recoverEtfRegistry(
  ...
  const completed = (await options.loadCompleted?.(stage)) ?? new Set<string>();
  let progress = 0;
    ...
    for (const gap of gaps) {
      ...
      if (gap.daily.length || gap.adjustment.length || gap.shareSize.length) {
        ...
        for (const code of result.missingDailyCodes) {
          ...
          await options.onItemComplete?.(stage, key);
          completed.add(key);
        }
      ...
      await options.onProgress?.(++progress, total);
    }
  }
