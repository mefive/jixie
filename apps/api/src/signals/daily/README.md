...
[CLI](../cli/run-signals.ts) 负责参数、退出码和 Prisma 收尾，用法以 [脚本入口说明](../../../scripts/README.md) 为准。改编排先对照 [Maintenance daily](../../application-maintenance/daily.ts) 的直接调用及 [runs](../runs/README.md)、[accounting](../accounting/README.md) 的集成测试；基础数据门槛与依赖期限分别看 runs/readiness 和 [rates.test.ts](../factor-inputs/rates.test.ts)。
