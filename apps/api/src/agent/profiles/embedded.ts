...
export function withEmbeddedAnalysis(
  ...
  return {
    ...profile,
    tools: [
      ...(profile.tools ?? []).filter((tool) => tool.name !== 'searchResearchCatalog'),
