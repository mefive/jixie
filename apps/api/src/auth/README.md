...
- `http/auth.ts`：保持 `/api/auth/*` 的 URL、参数校验与响应。
- `http/session.ts`：Cookie、鉴权中间件和 Hono 用户上下文声明。

现有顺序必须保留：验证码验证成功后先标记 consumed，再处理注册；创建用户和消费邀请码在同一事务内，Session 随后创建。未登录的 `/me` 返回 `{ user: null }`；受保护请求仍返回 401。生产不注册开发登录路由。

`http/auth.test.ts` 用全新临时 SQLite 和替代邮件传输验证注册、登录、重放、失败清理、事务回滚和 Cookie；`http/multi-user-permissions.test.ts` 保留跨用户业务权限回归。模块不依赖其他业务目录，HTTP 适配之外不导入 Hono。
