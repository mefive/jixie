...
export async function generateResearchFactorDraft(
  ...
  const context = researchFactorContext(execution);
...
function researchFactorContext(execution: ResearchExecutionV1): string {
  const sections = [
    JSON.stringify({
  ...
  for (const cell of execution.cells) {
    const source = clip(cell.source, MAX_CELL_SOURCE_CHARACTERS);
    const outputs = clip(
...
function researchOutputPreview(output: ResearchCellOutputBlockV1): unknown {
  switch (output.type) {
    case 'text':
      return { ...output, text: clip(output.text, 1_000) };
    case 'value':
      return output;
    case 'table':
      return {
