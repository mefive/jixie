...
it.each([
  '20260914090000_embedded_analysis_execution',
  '20260914110000_research_embedded_continuation',
])(
  'upgrades through %s without rewriting existing evidence',
  async (migration) => {
    ...
    try {
      ...
      deploy();
      expect(
        await database.researchDocument.findUnique({
          where: { id: 'document' },
          select: { embeddedSource: true, embeddedRunKey: true },
        }),
      ).toEqual({ embeddedSource: null, embeddedRunKey: null });
