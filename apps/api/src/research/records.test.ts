...
async function createFixtureSchema(database: PrismaClient) {
  const statements = [
    ...
    'CREATE TABLE "ResearchCellChangeProposal" ("id" TEXT NOT NULL PRIMARY KEY, "documentId" TEXT NOT NULL, "sourceTurnId" TEXT NOT NULL, "sourceMessageId" TEXT NOT NULL, "sourcePartIndex" INTEGER NOT NULL, "title" TEXT NOT NULL, "summary" TEXT NOT NULL, "expectedDocumentUpdatedAt" DATETIME NOT NULL, "expectedDocumentContentRevision" INTEGER, "appliedDocumentContentRevision" INTEGER, "operations" JSONB NOT NULL, "status" TEXT NOT NULL DEFAULT \'pending\', "conflict" JSONB, "reviewSessionId" TEXT, "reviewSequence" INTEGER, "reviewStatus" TEXT, "reviewIsLatest" BOOLEAN NOT NULL DEFAULT false, "reviewResolvedAt" DATETIME, "createdAt" DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP, "resolvedAt" DATETIME, FOREIGN KEY ("documentId") REFERENCES "ResearchDocument"("id") ON DELETE CASCADE, FOREIGN KEY ("sourceTurnId") REFERENCES "AgentTurn"("id") ON DELETE CASCADE, FOREIGN KEY ("sourceMessageId") REFERENCES "AgentMessage"("id") ON DELETE CASCADE)',
