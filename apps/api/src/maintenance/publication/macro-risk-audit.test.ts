...
import { selectMacroRiskAuditStart } from '#strategy/risk/data-readiness.js';
import { MACRO_RISK_AXIS_KEYS_V1, type RiskDataLineageV1 } from '@jixie/shared';
import { describe, expect, it } from 'vitest';
import { summarizeMacroRiskAxisQuality } from './risk-audit.js';
...
describe('macro-risk axis quality', () => {
  ...
  it('keeps the 36-observation model gate separate from valid macro lineage', () => {
    const exploratory = history(35, false, true);
    const strict = history(35, true, true);
    expect(baseMacroQuality(exploratory, strict)).toMatchObject({
      lineageErrors: [],
      exploratoryCompleteObservations: 35,
      strictCompleteObservations: 35,
    });
    expect(summarizeMacroRiskAxisQuality(exploratory, strict)).toMatchObject({
      status: 'error',
      errors: [
        'only 35 complete exploratory macro-axis changes are available',
        `insufficient exploratory axes: ${MACRO_RISK_AXIS_KEYS_V1.join(', ')}`,
      ],
      warnings: [
        'strict PIT history has 35/36 required complete observations while local vintages accumulate',
      ],
    });
    expect(
      summarizeMacroRiskAxisQuality(history(36, false, true), history(36, true, true)),
    ).toMatchObject({ status: 'pass', errors: [], warnings: [] });
  });
