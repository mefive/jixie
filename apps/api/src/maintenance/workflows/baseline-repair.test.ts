vi.mock('#market/calendar/read.js', () => ({
  getRecentOpenDates: dependencies.getRecentOpenDates,
}));
...
vi.mock('../runs/coordination.js', () => ({
  assertProductionLock: dependencies.assertProductionLock,
}));
...
vi.mock('#market/state/quality.js', () => ({
...
vi.mock('../runs/state.js', () => dependencies);
vi.mock('../publication/watermark.js', () => dependencies);
...
beforeEach(() => {
  ...
  dependencies.getRecentOpenDates.mockResolvedValue(['20260910', '20260911', '20260914']);
...
describe('baseline repair without publication', () => {
  it.each([
    ...
    expect(dependencies.getRecentOpenDates).toHaveBeenCalledWith(expectedCutoff, 3);
  ...
  it('rejects missing completed dates and empty calendars before creating a provider', async () => {
    ...
    dependencies.getRecentOpenDates.mockResolvedValueOnce([]);
  ...
  it('retains the default lookback and propagates derived validation failures', async () => {
    ...
    expect(dependencies.getRecentOpenDates).toHaveBeenCalledWith('20260914', 20);
