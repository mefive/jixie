...
import { type CommodityHoldingSyncSummary } from '#market/commodity/commodity-holding-positions.js';
import {
...
import { refreshCommodityHoldingPositions as refreshCommodityHoldingData } from '#market/commodity/holding-sync.js';
import {
    ...
    if (completedDates.length > 0) {
      ...
      const advancesWatermark =
        ...
        (state.dailyPublishedThrough != null &&
          (await isNextOpenDate(state.dailyPublishedThrough, readyThrough)));
...
async function initializePublishedBaseline(
  ...
  try {
    ...
    const dates = await getRecentOpenDates(tradeDate, lookback);
    ...
    const expectedDates =
      validationStart === tradeDate ? [tradeDate] : await getOpenDates(validationStart, tradeDate);
    await updateMaintenanceRun(run.id, 'validating_derived', summary);
...
async function refreshCommodityHoldingPositions(
  ...
  return refreshCommodityHoldingData(client, startDate, cutoff, onLog);
}
...
async function healPublishedTail(
  ...
  const dates = await getRecentOpenDates(publishedThrough, lookback);
  ...
  if (repaired.earliestDerivedChange) {
    ...
    const expectedDates = await getOpenDates(repaired.earliestDerivedChange, publishedThrough);
