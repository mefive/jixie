...
async function run(start,end){
  const post=await (await fetch(`${BASE}/api/app/backtest`,{method:'POST',headers:{'content-type':'application/json',cookie},body:JSON.stringify({name:'rep',start,end,initialCash:1000000,code})})).json();
  const t0=Date.now();
  for(let i=0;i<400;i++){await new Promise(r=>setTimeout(r,1500));const j=await(await fetch(`${BASE}/api/app/backtest/${post.jobId}?since=0`,{headers:{cookie}})).json();
    if(j.status!=='running'){const s=((Date.now()-t0)/1000).toFixed(0); console.log(`${start}~${end} (${s}s):`, j.result?`${j.result.trades} 笔, 收益 ${(j.result.totalReturn*100).toFixed(2)}%`:('ERR '+j.message)); return;}}
