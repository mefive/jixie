...
在现有全A、沪深300、中证500、中证1000和中证2000之外，后续按数据覆盖与使用价值逐步接入
更多宽基 / 风格指数；优先评估上证50、中证800、科创50、创业板指、中证A500及红利类指数。
接入门槛是同时具备可持续同步的官方指数行情与历史成分快照；历史计算必须使用不晚于观察日的
时点成分，缺少历史成分时从首个可信快照自然开始，禁止用当前成分倒推。指数数量增多后将观察
范围控件改为分组选择，并评估增加同屏风格对比视图；指数估值选择仍与市场脉搏范围保持独立。
